function     estimates =         lcss_mahony_attitude_observer(measurements, init_state, params) %
%             LCSS_MAHONY_ATTITUDE_OBSERVER Riccati -
%         based linear attitude observer(LCSS 2025) with Mahony coupling.% %
%             Implements the attitude observer from the LCSS 2025 paper using a %
%             vectorized rotation -
%         matrix state x = vec(R ^ T) ∈ R ^
%                          9. % Instead of projecting x back to SO(3) via SVD,
%     a separate SO(3) state is %
%             maintained and coupled to x using a Nonlinear Complementary
%             Filter(Mahony)
%                 .%
%             %
%             -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --%
...
%            .imu.a_B[3×N] Gravity vector in body frame
%            g_B = −R ^ T[0; 0; g] %.mag.time[1×M] Magnetometer timestamps(s) %
%                                       .mag.mag_meas[3×M] Magnetometer
%                                       readings(body frame) %
%                               % init_state
%                           -
%                           Struct with initial estimates
%     : %.R[3×3] Initial rotation matrix(SO(3)) %
%       % params -
%                           Struct with tuning parameters
...
%       -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --%
%       Outputs % estimates -
%                           Struct with attitude history
%     : %.R{1×N} Cell array of 3×3 rotation matrices
%       %.time[1×N] Time vector(= imu.time) %
%       %
%       -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --%
%       Notes %   • This is a pure
%                           - attitude observer;
% .P and.V are NOT populated.%   • The observer state x is unconstrained.The
%                                    attitude is extracted %
%         into a separate rotation matrix using a Mahony -
%     style NCF loop.%   • To compare cases,
%     call this function once per(C_acc, C_mag) pair and
% %     compute  trace(I - R_true{k} * R_hat{
%   k}')  in the calling script.
% --------------------------------------------------------------------------
    ...
    % Mahony coupling gain
    if isfield(params, 'k_att'),    k_att = params.k_att;
    else,                           k_att = 1.0; end
    ...
    for k = 1:N-1
        ...
        N_mat = -[ALLFUNCS.skew(x(1:3)); ...
         ...
         M_k   = 2000*dt * N_mat * Q_gyro * N_mat';% + 10 * eye(9);   
        % ----------------------------------------------------------------
        ...
        % ----------------------------------------------------------------
        % F. ATTITUDE RECONSTRUCTION (MAHONY / NCF)
        % ----------------------------------------------------------------
        % Extract pseudo-linear axes from x = vec(R^T)
        v1 = x(1:3);
        v2 = x(4:6);
        v3 = x(7:9);

        % Compute Mahony innovation
