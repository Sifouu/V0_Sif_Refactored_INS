...
To resolve this, we fundamentally redesigned the covariance mapping between the two stages. Recognizing the double integration, we significantly inflated the assumed measurement noise covariance (and equivalently, lowered the proportional correction gains) inside the Mahony stage. By essentially "distrusting" the dynamic transients of the LCSS output and increasing the process noise margin, the Mahony filter was forced to rely more heavily on its internal gyroscopic propagation for high-frequency dynamics, using the LCSS output strictly for low-frequency drift correction.
...
### 6.1 Unresolved Inherent Limitations
The cascaded Mahony approach remains fundamentally sub-optimal from an information-theoretic standpoint. Because the algorithm relies on cascading two distinct observers, cross-correlation terms between the linear state uncertainty and the nonlinear attitude uncertainty cannot be rigorously modeled. The tuning remains heuristic rather than strictly statistically optimal.
